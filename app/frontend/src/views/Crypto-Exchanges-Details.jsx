...
import {
  BsDribbble, BsFacebook, BsGithub,
} from 'react-icons/bs';
...
function CryptoExchangesDetails() {
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      headerAlign: 'center',
      renderCell: (params) => {
        const getTrustScoreColor = (value) => {
          if (value === 'green') return 'bg-green-500';
          if (value === 'yellow') return 'bg-yellow-500';
          return 'bg-red-500';
        };
        return (
          <div className={`w-3 h-3 rounded-full ${getTrustScoreColor(params.value)}`} />
        );
      },
    },
  ], []);
  ...
  const processedTickers = useMemo(() => {
    ...
    return exchangeDetails.tickers.map((ticker) => ({
      ...
      spread: `${(ticker.bid_ask_spread_percentage || 0).toFixed(2)}%`,
