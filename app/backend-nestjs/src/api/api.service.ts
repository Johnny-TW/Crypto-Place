...
export class ApiService {
  ...
  constructor(
    ...
    this.cryptocompareApiUrl = this.configService.get<string>('cryptocompareApiUrl');
  ...
  async getBitcoinMarketChart(query: any) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.coingeckoApiUrl}/coins/bitcoin/market_chart`, {
          params: query,
          headers: this.getCoingeckoHeaders(),
        }),
      );
  ...
  async getNews(query: any) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.cryptocompareApiUrl}/news/v1/article/list`, {
          params: query,
        }),
      );
  ...
  async getExchangeById(id: string) {
    try {
      ...
      this.logger.error(`Error fetching exchange data for ${id}:`, error.message);
      throw new HttpException(
        { error: error.message },
        error.response?.status || HttpStatus.INTERNAL_SERVER_ERROR,
      );
  ...
  async getExchangeTickersById(id: string) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.coingeckoApiUrl}/exchanges/${id}/tickers`, {
          headers: this.getCoingeckoHeaders(),
        }),
      );
      ...
      this.logger.error(`Error fetching exchange tickers for ${id}:`, error.message);
      throw new HttpException(
        { error: error.message },
        error.response?.status || HttpStatus.INTERNAL_SERVER_ERROR,
      );
